-- Reconciled from supabase_migrations.schema_migrations (version 20260820101452).

alter table am_application_forms add column if not exists description text;
...
as $$
  select case when f.id is null then null else jsonb_build_object(
    'id', f.id, 'slug', f.slug, 'title', f.title, 'description', f.description, 'is_enabled', f.is_enabled,
