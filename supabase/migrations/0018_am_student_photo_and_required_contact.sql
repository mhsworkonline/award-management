-- Reconciled from supabase_migrations.schema_migrations (version 20260821061826).

-- Storage bucket for student photographs. Public (read via plain URL, no
-- signing) so thumbnails render instantly in both the admin app and the
-- public /apply form; write access is scoped by RLS below.
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('am-student-photos', 'am-student-photos', true, 3145728, array['image/jpeg','image/png','image/webp'])
on conflict (id) do nothing;

create policy am_photos_anon_insert on storage.objects
  for insert to anon
  with check (bucket_id = 'am-student-photos');

create policy am_photos_authenticated_all on storage.objects
  for all to authenticated
  using (bucket_id = 'am-student-photos')
  with check (bucket_id = 'am-student-photos');

-- Photograph storage path — nullable at the DB level (existing rows predate
-- this field) but required by app-layer validation on every new create/submit.
alter table am_students add column if not exists photo_path text;
alter table am_public_submissions add column if not exists photo_path text;

-- Contact number and photograph become required going forward; both are
-- validated app-side too, but the security-definer entry point for anon
-- submissions re-checks server-side same as every other required field here.
create or replace function public.am_submit_public_application(
  ...
  p_ip_hash text, p_photo_path text
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  insert into am_public_submissions (
    ...
    lower(trim(p_email)), trim(p_contact_no),
