-- Award Management — middle name (father's/husband's name) becomes required
-- on the public application form, same as first/last name, email, contact
-- number and photograph. Re-validated here since am_submit_public_application
-- is a SECURITY DEFINER function callable by anon — client-side validation
-- alone is never the real gate.
--
-- Scoped to the public form only. am_students (admin "Add student") and
-- am_public_submissions' staff-edit path are untouched — some already-
-- approved/legacy records may not have one, and forcing it there could break
-- editing them.
