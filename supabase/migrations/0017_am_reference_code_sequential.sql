-- Reconciled from supabase_migrations.schema_migrations (version 20260820114910).
...
begin
  ...
  -- Reference code: "S" + 2-digit academic-year + a plain sequential number,
  -- e.g. S25-1, S25-2 ... S25-137 — restarts at 1 each academic year.
  -- The advisory lock (namespaced so it can't collide with other apps sharing
  -- this Postgres instance) serializes concurrent submissions in the same
  -- year so two applicants can never be handed the same number.
