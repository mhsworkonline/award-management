-- Staff-entered applications. A staff member with Submissions: Create can add
-- an application through the same form students use, even when that form is
-- disabled (closed to the public) — the app's private /forms/[id]/add route.
-- Adds a trailing p_staff flag to am_submit_public_application; it is only
-- honored for a signed-in user holding that permission (anon passing it just
-- gets an error), and it skips the "form enabled" gate and the per-device
-- rate limit. Everything else — validation, reference-code minting — is
-- unchanged, so staff entries land as ordinary Pending submissions.

drop function if exists am_submit_public_application(uuid,uuid,text,text,text,text,text,text,uuid,text,uuid,text,uuid,uuid,uuid,text,text,integer,text,numeric,text,text,text,text,text,uuid);
...
begin
  if coalesce(p_staff, false) then
    if auth.uid() is null or not am_has_permission('submissions', 'create') then
      raise exception 'You do not have permission to add applications';
    end if;
    v_staff := true;
  end if;
  ...
  if not v_enabled and not v_staff then
  ...
  if p_ip_hash is not null and not v_staff then
...
revoke all on function am_submit_public_application(uuid,uuid,text,text,text,text,text,text,uuid,text,uuid,text,uuid,uuid,uuid,text,text,integer,text,numeric,text,text,text,text,text,uuid,boolean) from public;
grant execute on function am_submit_public_application(uuid,uuid,text,text,text,text,text,text,uuid,text,uuid,text,uuid,uuid,uuid,text,text,integer,text,numeric,text,text,text,text,text,uuid,boolean) to anon, authenticated;
