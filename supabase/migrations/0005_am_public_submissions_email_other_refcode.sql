-- Reconciled from supabase_migrations.schema_migrations (version 20260820081501).

-- 1. Email joins contact_no as persistent identity info.
alter table am_students add column if not exists email text;

-- 2. Submissions: email, free-text "Other" institution/course, reference code.
alter table am_public_submissions add column if not exists email text;
alter table am_public_submissions alter column institution_id drop not null;
alter table am_public_submissions add column if not exists other_institution_name text;
alter table am_public_submissions add column if not exists other_course_name text;
alter table am_public_submissions add column if not exists other_course_structure text
  check (other_course_structure in ('year','semester'));
alter table am_public_submissions add column if not exists reference_code text;
alter table am_public_submissions alter column reference_code set not null;
alter table am_public_submissions add constraint am_public_submissions_refcode_uk unique (reference_code);

alter table am_public_submissions drop constraint if exists am_public_submissions_placement_ck;
alter table am_public_submissions add constraint am_public_submissions_placement_ck check (
  standard_id is not null or course_id is not null or other_course_name is not null
);
alter table am_public_submissions add constraint am_public_submissions_institution_ck check (
  institution_id is not null or other_institution_name is not null
);

create index am_public_submissions_refcode_idx on am_public_submissions (reference_code);

-- 3. Rebuild the submit function: email, Other institution/course, reference code.
drop function if exists am_submit_public_application(uuid,text,text,text,text,uuid,uuid,uuid,int,text,numeric,text,text);
...
begin
  ...
  if p_standard_id is null and p_course_id is null then
    ...
    if p_other_course_structure not in ('year','semester') then
...
revoke all on function am_submit_public_application(uuid,text,text,text,text,text,uuid,text,uuid,uuid,text,text,int,text,numeric,text,text) from public;
grant execute on function am_submit_public_application(uuid,text,text,text,text,text,uuid,text,uuid,uuid,text,text,int,text,numeric,text,text) to anon, authenticated;
