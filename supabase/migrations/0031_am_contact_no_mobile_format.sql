-- Award Management — enforce a 10-digit Indian mobile number for the public
-- form's contact number, at the database layer too — same treatment email
-- already gets a few lines below (non-empty AND a valid shape, not just
-- non-empty). The Zod schema (lib/validators.ts' indianMobileNo) already
-- gates this before the RPC is ever called through the app, but this
-- function is SECURITY DEFINER and directly callable by anon, so app-level
-- validation alone is never the real gate.
--
-- Signature is unchanged from 0028 — a plain create or replace is enough,
-- no need to drop the old overload first (see 0020/0028 for when that
-- actually is needed: an added/removed parameter, not just a body change).
