...
import { commitInstitutionImport } from "@/lib/actions/institutions";
import type { InstitutionImportType } from "@/lib/excel/institutions-workbook";
import type { ParsedInstitutionRow } from "@/app/api/institutions/import/route";
...
/** Shared by both /institutions/import/school and /institutions/import/college
 *  — same flow, the `type` prop is what actually differs (columns shown,
 *  template downloaded, which API routes are hit). No institution/year
 *  "destination" step needed here, unlike the student importer — the whole
 *  point of this import *is* creating the destination rows. */
export function InstitutionImportWizard({ type }: { type: InstitutionImportType }) {
  const router = useRouter();
  const label = type === "school" ? "school" : "college";
  const Label = type === "school" ? "School" : "College";
  ...
  const [rows, setRows] = React.useState<ParsedInstitutionRow[]>([]);
  ...
  async function upload() {
    ...
    try {
      const res = await fetch("/api/institutions/import", { method: "POST", body });
      ...
      const parsedRows = json.rows as ParsedInstitutionRow[];
      setRows(parsedRows);
      setSummary(json.summary as Summary);
      // Pre-select every clean row except ones already in the database —
      // that's a real duplicate here (unlike students, there's no
      // "enrollment" concept to attach to), so it stays opt-in.
      setSelected(
        new Set(
          parsedRows.filter((r) => r.errors.length === 0 && !r.duplicate).map((r) => r.rowNumber),
        ),
      );
      setStep("review");
    } catch (e) {
  ...
  async function commit() {
    ...
    setCommitting(true);
    const result = await commitInstitutionImport({
      type,
      rows: payload.map((r) => ({
        name: r.name,
        board_id: r.board_id,
  ...
  if (step === "done") {
    return (
      <>
        <PageHeader title={`Import ${label}s`} />
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-14 text-center">
            ...
            <p className="text-base font-semibold">
              Imported {inserted.toLocaleString("en-IN")} {label}
              {inserted === 1 ? "" : "s"}
            </p>
            {skipped > 0 && (
              <p className="max-w-sm text-[13px] text-muted-foreground">
                {skipped} row{skipped === 1 ? "" : "s"} skipped — already existed by the time the import ran.
              </p>
            )}
        ...
        actions={
          <>
            <Button asChild variant="outline">
              <a href={`/api/institutions/template?type=${type}`}>
                <Download /> Template
              </a>
            </Button>
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="flex justify-end">
              <Button onClick={() => void upload()} disabled={!file || parsing}>
      ...
      {step === "review" && summary && (
        <>
          <div className="grid gap-3 sm:grid-cols-5">
            ...
            <Tile label="Already exists" value={summary.alreadyExists} />
          ...
          {(summary.duplicates > 0 || summary.alreadyExists > 0) && (
            <p className="flex items-start gap-2 rounded-md border border-warning/40 bg-warning/8 px-3.5 py-2.5 text-[13px]">
              <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-warning" />
              <span>
                {summary.duplicates + summary.alreadyExists} possible duplicate
                {summary.duplicates + summary.alreadyExists === 1 ? " was" : "s were"} left unselected.
                Tick one to import it anyway.
              </span>
            </p>
          )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="text-muted-foreground">
                        {row.contact_person || row.contact_no ? (
                          <span className="flex flex-col leading-tight">
                            {row.contact_person && <span>{row.contact_person}</span>}
                            {row.contact_no && <span className="tabular text-[12px]">{row.contact_no}</span>}
                          </span>
                        ) : (
                          "—"
                        )}
                      ...
                      <TableCell>
                        {blocked ? (
                          <span className="flex flex-col gap-0.5">
                            {row.errors.map((e) => (
                              <span key={e} className="text-[12px] font-medium leading-snug text-destructive">
                                {e}
                              </span>
                            ))}
                          </span>
                          ...
                          <span className="flex items-center gap-1.5">
                            ...
                            <span className="text-[12px] leading-snug text-warning">{row.duplicate.detail}</span>
                          </span>
                        ) : (
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={() => void commit()} disabled={committing || selected.size === 0}>
                ...
                {committing ? "Importing…" : `Import ${selected.size} ${label}${selected.size === 1 ? "" : "s"}`}
              </Button>
            </div>
          </div>
        </>
    ...
    <ol className="flex items-center gap-2 text-[13px]">
      {items.map((item, index) => (
        <li key={item.key} className="flex items-center gap-2">
          ...
          <span className={index <= activeIndex ? "font-medium" : "text-muted-foreground"}>{item.label}</span>
...
function Tile({
  ...
  return (
    <div className="rounded-lg border bg-card p-4 shadow-soft">
      ...
      <p className={`tabular mt-1 text-xl font-semibold leading-none ${tone}`}>{value.toLocaleString("en-IN")}</p>
    </div>
  );
}
